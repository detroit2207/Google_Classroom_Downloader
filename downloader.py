...
def safe_execute(request, description="request"):
    while True:
        ...
        except (socket.gaierror, RemoteDisconnected) as e:
            print(f"⚠️ Network issue during {description}: {e}")
            print("🔄 Waiting for internet connection to restore...")
            time.sleep(5)
        except HttpError as e:
            print(f"⚠️ HTTP error during {description}: {e}")
            print("🔄 Retrying in 5 seconds...")
            time.sleep(5)
        except Exception as e:
            print(f"⚠️ Unexpected error during {description}: {e}")
            print("🔄 Retrying in 5 seconds...")
            time.sleep(5)
...
def get_credentials_path():
    ...
    print(creds_path)  # Debugging line to check path
    return creds_path
...
def download_drive_file(file_id, file_path, drive_service, total_files=None, current_index=None):
    ...
    if os.path.exists(file_path):
        if total_files and current_index:
            ...
            # Matches Electron regex so bar updates
...
def download_from_drive_folder(drive_service, folder_id, out_dir, total_files, current_index):
    """Recursively download all files in a Drive folder"""
...
def count_files_in_drive_folder(drive_service, folder_id):
    """Count all files inside a Drive folder recursively"""
    query = f"'{folder_id}' in parents and trashed = false"
    page_token = None
    total = 0
    while True:
...
def count_total_files(classroom_service, drive_service, course_id):
    """Count all downloadable files in a course"""
    total = 0
        ...
        for m in a.get("materials", []):
            if "driveFile" in m:
                ...
                if df.get("mimeType") == "application/vnd.google-apps.folder":
                    ...
                    dest = os.path.join(folder_dir, safe_name(df.get("title") or df.get("name") or df["id"]))
